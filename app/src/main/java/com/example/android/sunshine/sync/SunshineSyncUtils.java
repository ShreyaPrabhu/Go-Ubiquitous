...
public class SunshineSyncUtils {
    ...
    static void scheduleFirebaseJobDispatcherSync(@NonNull final Context context) {
        ...
        //notifyWear(context);
        //Log.v(SUNSHINE_SYNC_TAG, "called2");
    }
    /**
    ...
    public static void startImmediateSync(@NonNull final Context context) {
        ...
        //notifyWear(context);
        //Log.v(SUNSHINE_SYNC_TAG, "called");
    }

    /*public static void notifyWear(Context context) {
        Log.v(SUNSHINE_SYNC_TAG, "inside");
        Uri weatherUri = WeatherContract.WeatherEntry
                .buildWeatherUriWithDate(System.currentTimeMillis());
        Log.v(SUNSHINE_SYNC_TAG, "weatherUri" + weatherUri);
        Cursor cursor = context.getContentResolver()
                .query(weatherUri, MainActivity.MAIN_FORECAST_PROJECTION, null, null, null);
        Log.v(SUNSHINE_SYNC_TAG, "weacursor" + cursor +"/"+cursor.moveToFirst() + "/"+cursor.getCount() );
        if (cursor!=null && cursor.getCount()>0) {
            Log.v(SUNSHINE_SYNC_TAG, "inside" + "in1");
            cursor.moveToFirst();
                    ...
                    .setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
                        ...
                        public void onResult(DataApi.DataItemResult result) {
                            if (!result.getStatus().isSuccess()) {
                                Log.v(SUNSHINE_SYNC_TAG, "cant"+"Cannot send weather information, status code: "
                                        + result.getStatus().getStatusCode());
                            } else {
                                Log.v(SUNSHINE_SYNC_TAG, "can"+"Weather information was sent successfully "
